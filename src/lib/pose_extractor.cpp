...
std::vector<std::vector<int>> get_poses(cv::Mat *img, cv::dnn::Net *network_pose,
                                        ...
                                        float scale_blob,
                                        bool filter_poses, int min_keypoint)
{
    // get image size
                             ...
                             std::vector<std::vector<int>> *poses, cv::Mat *img)
{
    ...
    for (int n = 0; n < poses->size(); n++)
    {
        ...
        // cv::rectangle((*img), box_pose, Scalar(255, 255, 0), 2);
        // cv::rectangle((*img), box_pose & (*ROI), Scalar(255, 160, 40),-1,8);
        // cv::rectangle((*img), (*ROI), Scalar(0, 160, 40),3,8);

        // evaluate a score: rate of intersecated rect over area of ROI
        // cout << "\n"
        //      << n << " - pose score " << score << " -- box: " << box_pose.tl() << "-" << box_pose.br() << " -- area: " << box_pose.area();
        float score = (float)(box_pose & (*ROI)).area() / (float)box_pose.area();
        bool tmp = (box_pose | (*ROI)).area() < (box_pose.area() + (*ROI).area());

        // cout << "\n"
        //      << n << "box area > 0 flag: " << ((box_pose & (*ROI)).area() > 0) << "score " << score << "\n";
        // cout << "counter critic points" << counter_critic_point << "\n";
